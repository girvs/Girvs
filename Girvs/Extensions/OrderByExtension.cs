...
public static class OrderByExtension
{
    ...
    static IQueryable<T> _OrderBy<T>(IQueryable<T> query, string propertyName, bool isDesc)
    {
        ...
        var method = typeof(OrderByExtension)
    ...
    public static IQueryable<T> OrderByInternal<T, TProp>(
        IQueryable<T> query,
        PropertyInfo memberProperty
    ) => query.OrderBy(_GetLamba<T, TProp>(memberProperty));

    public static IQueryable<T> OrderByDescendingInternal<T, TProp>(
        IQueryable<T> query,
        PropertyInfo memberProperty
    ) => query.OrderByDescending(_GetLamba<T, TProp>(memberProperty));
    ...
    static Expression<Func<T, TProp>> _GetLamba<T, TProp>(PropertyInfo memberProperty)
    {
        ...
        var lambda = Expression.Lambda<Func<T, TProp>>(
