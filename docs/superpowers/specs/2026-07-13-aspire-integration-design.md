...
- 日期：2026-07-13（2026-07-14 修订：新增 AppHost 自动编排；同日再修订：连接串覆盖逻辑下沉到各组件模块）
- 状态：已确认
- 架构图：仓库根目录 `Architect.png`（Girvs + .NET Aspire 模块化云原生架构图，六层：访问入口 / API Gateway / Aspire 编排与运行时 / 业务服务 / Girvs 基础组件能力 / 基础设施资源）
- 相关模块：新增 `Girvs.Aspire`、`Girvs.Aspire.Hosting`；涉及 `Girvs`（宿主入口 + `DependsOnAttribute`）、`Girvs.Consul`（共存，不改代码）、`Girvs.Cache`/`Girvs.EventBus`/`Girvs.EntityFrameworkCore`（各自新增 `ApplyAspireConnectionString(s)` 方法）
...
| 连接串覆盖归属 | **（2026-07-14 再修订）**由各组件模块自己实现并调用（如 `CacheConfig.ApplyAspireConnectionString`），不在 `Girvs.Aspire` 集中反射映射。理由：反射按属性名字符串写入是脆弱设计，且该逻辑本就该跟着它要修改的配置类走 |
...
**（2）连接串覆盖：由各组件自己实现（2026-07-14 修订）**

最初设计曾把 AppHost 注入的连接串映射集中放在 `Girvs.Aspire`，通过 `AppSettings.ModuleConfigurations` 字典 + 反射按属性名写入 EFCore/Cache/EventBus 的配置对象，以避免 `Girvs.Aspire` 对这些模块产生硬引用。评审认为这是坏味道：属性名以字符串硬编码、改名不会在编译期报错，且该反射逻辑与其要修改的配置类完全脱节。

修订为：**每个组件模块自己实现并调用** Aspire 连接串覆盖逻辑，与其自身配置类放在一起：

| Aspire 资源名（约定） | 归属模块 | 方法 |
|----------------------|----------|------|
| `girvs-db-<name>` / `girvs-db-<name>-read-<N>`（N 从 0 起连续） | `Girvs.EntityFrameworkCore` | `DataConnectionConfig.ApplyAspireConnectionStrings(IConfiguration)`（按 `Name` 匹配），`DbConfig.ApplyAspireConnectionStrings(IConfiguration)` 遍历调用 |
| `girvs-cache` | `Girvs.Cache` | `CacheConfig.ApplyAspireConnectionString(IConfiguration)` |
| `girvs-eventbus-rabbitmq`（AMQP URI）/`girvs-eventbus-redis`/`girvs-eventbus-db` | `Girvs.EventBus` | `EventBusConfig.ApplyAspireConnectionStrings(IConfiguration)`；AMQP URI 解析在 `RabbitMQConfig.ApplyAspireConnectionString(string)` |

各模块在自己的 `IAppModuleStartup.ConfigureServices` 中、拿到自身配置对象后立即调用该方法（如 `GirvsCacheModule.ConfigureServices` 里 `cacheConfig.ApplyAspireConnectionString(configuration)`），零新增包依赖——`IConfiguration.GetConnectionString` 是各模块已经在用的 `Microsoft.Extensions.Configuration` 扩展方法。只在对应连接串存在时覆盖，不存在时保持 appsettings.json 原值。业务代码与现有配置文件零改动。

`Girvs.Aspire` 不再持有、也不再依赖任何连接串映射逻辑，`AspireModule.Order` 不再需要"先于其他模块"的排序语义（各组件只覆盖自己的配置，互不影响顺序）。
...
- `tests/` 下新增 `Girvs.Aspire.Tests`（net10.0）：
  - 各组件的 `ApplyAspireConnectionString(s)` 方法：资源名 → 配置属性映射、无对应连接串时不覆盖原值、读库列表多条映射（直接构造 `CacheConfig`/`EventBusConfig`/`DbConfig` 调用，无需 DI）；
...
- 不改 Quartz 模块内部代码（`DependsOnAttribute` 加在核心包，组件模块本身不加声明；Cache/EventBus/EFCore 例外——各自新增了 `ApplyAspireConnectionString(s)` 方法，见决策表"连接串覆盖归属"）；
...
| Serilog 桥接的反射探测挂接点脆弱（`GirvsAspireSerilogHook`，仅此一处保留反射） | 挂接契约集中在一个静态类型名 + 方法名常量，加测试覆盖 |
| Serilog OTLP sink 与 `ReadFrom.Configuration` 的叠加顺序 | 桥接以代码方式追加 sink，不依赖 json 配置，验证两者共存 |
| Aspire 注入的连接串格式与 Girvs 期望格式不一致（如 Redis 带密码格式） | 各组件的 `ApplyAspireConnectionString(s)` 方法内做格式归一化，测试覆盖常见格式 |
