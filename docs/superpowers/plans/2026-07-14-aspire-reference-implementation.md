...
>
> **✅ EventBus 增量已完成并跑通（深层：CAP 真实发布 + 端到端消费）**：ServiceB 声明 `[DependsOn(EventBusModule, GirvsEntityFrameworkCoreModule)]`（CAP 存储需库，故同时声明 EFCore 让 Aspire 建 MySQL 库，`DbConnectionString="default"` 复用该连接）。Aspire 自动拉起 RabbitMQ + MySQL 容器，CAP 自动建 `cap.published`/`cap.received` 表。`/selfcheck/eventbus` 发布消息（cap.published=Succeeded、投递到 RabbitMQ），订阅者 `/selfcheck/eventbus/received` 收到原消息，端到端投递成功。此过程排查出 **4 处问题、修了 2 个真实框架 bug**：
> 1. **【框架 bug，已修+单测】枚举数字化 vs 贡献器字符串匹配**：框架启动会把服务 appsettings 的枚举回写为**数值**（`UseDataType:"MySql"→1`、`EventBusType:"RabbitMQ"→0`、`DistributedCacheType:"Redis"→2`），而 `Girvs.Aspire.Hosting` 的 `DatabaseResourceContributor`/`EventBusResourceContributor` 按**小写名称** switch（`"mysql"`/`"rabbitmq"`），数值匹配不上→跳过、不建资源→连接串不注入→CAP 连占位串超时。这也解释了"首次 EFCore 能跑、二次跑挂"（首次 appsettings 还是字符串，运行后被改成数字）。修法：两个贡献器加 `Normalize*` 兼容数字与名称（`Girvs.Aspire.Hosting/Contributors/*.cs`），并加数字枚举单测。
> 2. **【框架 bug，已修+单测】RabbitMQ vhost 默认值**：`RabbitMQConfig.ApplyAspireConnectionString` 在 AMQP URI 无 vhost 路径时**不覆盖** VirtualHost，保留了 appsettings 默认的 `zhuofan.wb`；而 Aspire 本地 broker 只有默认 vhost `/`→连接失败、cap.published=Failed（重试 3 次）。修法：URI 路径为空时按标准语义置 `/`（`Girvs.EventBus/Configuration/EventBusConfig.cs`），加单测。
> 3. **【样例组合，已注册 stub】CapEventBus 依赖 ClaimManager**：`CapEventBus.PublishAsync` 访问 `EngineContext.Current.ClaimManager.IdentityClaim`，其 `?.` 只保护 IdentityClaim 未保护 ClaimManager 本身；样例无认证模块→ClaimManager 为 null→NRE。真实服务由 `Girvs.AuthorizePermission` 提供，样例注册最小 `SampleClaimManager` 隔离验证。（附带发现：该 `?.` 位置是框架潜在脆弱点，可后续加固。）
> 4. **【样例疏漏，已修】订阅者 CapHeader 缺 `[FromCap]`**：消费端 `SubscriberExecutionFailedException: Deserialization of ... Type 'DotNetCore.CAP.CapHeader'`——CAP 试图从消息体反序列化 `CapHeader` 参数。真实 Girvs handler 均标注 `[FromCap] CapHeader header`，样例漏了。加上即消费成功。
>
> **调试环境提示（对迁移/开发机重要）**：① 用户配置 docker 代理后 shell 全局有 `http_proxy/all_proxy` 但无 `no_proxy`，Aspire 大量走 localhost，启动 AppHost 须带 `no_proxy=localhost,127.0.0.1,::1`；② 服务端框架会把枚举回写成数值（见上），排查 appsettings 时注意。
>
