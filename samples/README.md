...
| `Sample.Gateway` | 本地网关：通过 Consul 发现 ServiceA/ServiceB，并按 `/{service}/...` 约定转发 | Aspire.Gateway |
...
前置：本机 Docker 可拉取 `redis`/`mysql`/`rabbitmq`/`hashicorp/consul` 镜像（国内需配镜像加速/代理）。
...
启动后控制台会打印 Aspire Dashboard 地址。AppHost 拉起 Redis / MySQL / RabbitMQ / Consul 容器后,把实际地址就地更新进 `Sample.AppHost/girvs.shared.json`(不存在则创建,存在则只更新 `Resources` 节点)并以 `GIRVS_SHARED_CONFIG` 注入各服务;服务按自己配置中的 `ConnectionRef` 从 `Resources` 组装连接串。ServiceA/ServiceB 通过 `Girvs.Consul` 注册到 Consul，Sample.Gateway 再通过 `Girvs.Aspire.Gateway` 的 Consul 源生成 YARP 路由。
...
| Gateway | `GET /service-a/selfcheck/config` | 网关经 Consul 发现转发到 ServiceA | `{"logLevel":"Information","jwtSecretPresent":false}` |
| Gateway | `GET /service-b/ping` | 网关经 Consul 发现转发到 ServiceB | `pong` |
...
- 网关示例保留 Consul 部署形态：ServiceA/ServiceB 使用 `Girvs.Consul` 注册服务，Sample.Gateway 使用 `GatewayDiscoveryType.Consul` 轮询 Consul Agent 服务目录。
